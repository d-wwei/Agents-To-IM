# CLAUDE.md — Project Guidelines for agent-to-im-skill
...
>
> ```
> 请帮我排查 agent-to-im 桥接服务的问题。
> 1. 读取 ~/.agent-to-im/logs/bridge.log 最近 50 行日志
> 2. 读取 ~/.agent-to-im/config.env 检查配置是否正确
> 3. 运行 bash ~/.claude/skills/agent-to-im/scripts/doctor.sh 并分析输出
> 4. 根据日志和配置给出具体的修复建议
> ```
