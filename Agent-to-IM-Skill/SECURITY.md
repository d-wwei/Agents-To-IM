...
All credentials are stored in `~/.agent-to-im/config.env` with file permissions set to `600` (owner read/write only). This file is created during `setup` and never committed to version control.
...
3. Run `/agent-to-im reconfigure` to update the stored credentials
4. Run `/agent-to-im stop` then `/agent-to-im start` to apply changes
...
2. Run `/agent-to-im stop` to halt the daemon
3. Run `/agent-to-im reconfigure` with a new token
4. Review `~/.agent-to-im/logs/` for unauthorized activity
5. Run `/agent-to-im start` with the new credentials
