...
- **Feishu**: App ID → confirm → App Secret → confirm (masked) → Domain (optional) → Allowed User IDs (optional). After collecting credentials, explain the two-phase setup the user must complete:
  ...
  - **Phase 2** (requires running bridge): (D) run `/agent-to-im start`, (E) configure events (`im.message.receive_v1`) and callback (`card.action.trigger`) with long connection mode, (F) publish second version + admin approve.
...
- **Weixin**: Do not ask for a static token. Instead:
  ...
  3. The helper writes `~/.agent-to-im/runtime/weixin-login.html` and tries to open it automatically in the local browser.
  ...
  - Explain briefly: the linked Weixin account is stored in `~/.agent-to-im/data/weixin-accounts.json`. Running the helper again replaces the previously linked account.
