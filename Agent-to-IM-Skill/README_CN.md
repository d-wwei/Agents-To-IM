...
将当前安装的 AI 编程宿主桥接到 IM 平台 —— 在 Telegram、Discord、飞书、QQ 或微信中与 AI 编程代理对话。
...
- **权限控制** — Claude 支持逐工具内联审批（Telegram/Discord）或文本 `/perm` 命令 / 快捷 `1/2/3` 回复（飞书/QQ/微信）；Codex 在 `approval_policy=on-request` 时支持 IM 中的单轮前置审批
...
将仓库直接克隆到所选宿主的 Skills 目录，Claude Code 会自动发现。

#### 备选：符号链接方式（适合开发）

```bash
git clone https://github.com/d-wwei/Agents-To-IM.git ~/code/Agent-to-IM-Skill
mkdir -p ~/.codex/skills
ln -s ~/code/Agent-to-IM-Skill ~/.codex/skills/codex-to-im
```
...
```bash
git clone https://github.com/d-wwei/Agents-To-IM.git ~/code/Agent-to-IM-Skill
bash ~/code/Agent-to-IM-Skill/scripts/install-codex.sh
```

如果你想保留可开发的本地仓库：

```bash
bash ~/code/Agent-to-IM-Skill/scripts/install-codex.sh --link
```
...
```bash
bash ~/code/Agent-to-IM-Skill/scripts/install-host.sh --host claude
bash ~/code/Agent-to-IM-Skill/scripts/install-host.sh --host codex
bash ~/code/Agent-to-IM-Skill/scripts/install-host.sh --host gemini
```
...
**Codex：** 启动新会话，说 `codex-to-im setup`、`start bridge` 或 `帮我接微信桥接`。Codex 会识别 Skill 并使用 `~/.codex-to-im` 作为运行时目录。
...
打开 IM 应用，给你的机器人发消息，当前安装的宿主代理会通过桥接回复。

权限行为取决于 runtime：

- **Claude runtime** — 工具调用可在聊天中逐条审批：内联按钮 **允许** / **拒绝**（Telegram/Discord），或文本 `/perm` 命令 / 快捷 `1/2/3` 回复（飞书/QQ/微信）
