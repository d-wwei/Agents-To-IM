...
const LOCAL_SECRETS_INCLUDE_COMMENT = "# Load locally rotated secrets from a separate file so they do not need to live\n"
    + "# in the main bridge config.\n";
const LOCAL_SECRETS_INCLUDE_LINE = '[ -f "$HOME/.codex-to-im/openai.local.env" ] && source "$HOME/.codex-to-im/openai.local.env"';
...
function extractIncludedEnvPath(line) {
...
function loadEnvFile(filePath, seen = new Set()) {
 ...
 */
export function normalizeMsysPath(p) {
    if (process.platform !== 'win32')
        return p;
    const m = p.match(/^\/([a-zA-Z])\/(.*)/);
    if (m)
        return m[1].toUpperCase() + ':' + path.sep + m[2].split('/').join(path.sep);
    return p;
}
/**
 ...
 */
export function expandShellVars(value) {
    return value
...
export function loadConfig() {
    ...
    const hostDefaultRuntime = HOST_PROFILE.host === "codex" ? "codex"
        : HOST_PROFILE.host === "gemini" ? "gemini"
            : "claude";
    const rawRuntime = env.get("CTI_RUNTIME") || hostDefaultRuntime;
    const runtime = (["claude", "codex", "gemini", "auto"].includes(rawRuntime) ? rawRuntime : hostDefaultRuntime);
    return {
        runtime,
...
export function saveConfig(config) {
    ...
    out += formatEnvLine("CTI_CODEX_SKIP_GIT_REPO_CHECK", config.codexSkipGitRepoCheck === false ? "false" : "true");
    ...
    out += formatEnvLine("CTI_FEISHU_AUDIO_TRANSCRIBE", config.feishuAudioTranscribe === false ? "false" : "true");
    ...
    out += formatEnvLine("CTI_DISCORD_ALLOWED_USERS", config.discordAllowedUsers?.join(","));
    out += formatEnvLine("CTI_DISCORD_ALLOWED_CHANNELS", config.discordAllowedChannels?.join(","));
    out += formatEnvLine("CTI_DISCORD_ALLOWED_GUILDS", config.discordAllowedGuilds?.join(","));
    out += formatEnvLine("CTI_QQ_APP_ID", config.qqAppId);
    out += formatEnvLine("CTI_QQ_APP_SECRET", config.qqAppSecret);
    out += formatEnvLine("CTI_QQ_ALLOWED_USERS", config.qqAllowedUsers?.join(","));
...
export function configToSettings(config) {
    ...
    if (config.discordAllowedChannels)
        m.set("bridge_discord_allowed_channels", config.discordAllowedChannels.join(","));
    if (config.discordAllowedGuilds)
        m.set("bridge_discord_allowed_guilds", config.discordAllowedGuilds.join(","));
    // ── Feishu ──
    ...
    if (config.feishuAppId)
        m.set("bridge_feishu_app_id", config.feishuAppId);
    if (config.feishuAppSecret)
        m.set("bridge_feishu_app_secret", config.feishuAppSecret);
    if (config.feishuDomain)
        m.set("bridge_feishu_domain", config.feishuDomain);
    if (config.feishuAllowedUsers)
        m.set("bridge_feishu_allowed_users", config.feishuAllowedUsers.join(","));
    m.set("bridge_feishu_audio_transcribe", config.feishuAudioTranscribe === false ? "false" : "true");
    ...
    if (config.qqAppId)
        m.set("bridge_qq_app_id", config.qqAppId);
    if (config.qqAppSecret)
        m.set("bridge_qq_app_secret", config.qqAppSecret);
    ...
    m.set("bridge_weixin_enabled", config.enabledChannels.includes("weixin") ? "true" : "false");
    if (config.weixinMediaEnabled !== undefined)
