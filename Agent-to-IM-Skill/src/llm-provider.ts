 ...
 * Returns { ok, version?, error?, pollution? }.
 */
export function preflightCheck(cliPath: string): { ok: boolean; version?: string; error?: string; pollution?: string } {
  ...
  const pollution = detectStdoutPollution(cliPath, cleanEnv);
  return { ok: true, version: compat.version, pollution };
}

// ── CLI stdout pollution detection & wrapper ──

/**
 * After getting the version string via getCliVersion(), also check for stdout noise.
 * Some custom CLI forks (e.g., enterprise proxy wrappers) print non-JSON output
 * to stdout during initialization which corrupts the SDK's JSON stream.
 */
function detectStdoutPollution(cliPath: string, env?: Record<string, string>): string | undefined {
  try {
    const cmd = (process.platform === 'win32' && /\.(js|mjs)$/i.test(cliPath))
      ? `"${process.execPath}" "${cliPath}" --version`
      : `"${cliPath}" --version`;
    ...
    // Version output should be a single line like "2.1.62 (Claude Code)"
    const lines = output.split('\n').filter(l => l.trim());
    if (lines.length > 1) {
      // Extra lines beyond the version are pollution
      const noise = lines.filter(l => !/^\d+\.\d+/.test(l.trim()));
      if (noise.length > 0) return noise[0].trim();
    }
    return undefined;
  } catch {
    return undefined;
...
/**
 * Generate a wrapper script that suppresses known stdout noise from the CLI.
 * Returns the path to the generated wrapper.
 */
export function generateCliWrapper(cliPath: string, suppressPatterns: string[]): string {
  const home = process.env.CTI_HOME || path.join(os.homedir(), '.claude-to-im');
  fs.mkdirSync(home, { recursive: true });

  // Write prelude script that intercepts console.log for known noise patterns
  const preludePath = path.join(home, 'suppress-cli-noise.mjs');
  const patterns = JSON.stringify(suppressPatterns);
  fs.writeFileSync(preludePath, `\
// Auto-generated by claude-to-im to suppress CLI stdout noise
const _log = console.log;
const _patterns = ${patterns};
const _suppressed = new Set();
console.log = (...args) => {
  if (args.length === 1 && typeof args[0] === 'string' && _patterns.includes(args[0]) && !_suppressed.has(args[0])) {
    _suppressed.add(args[0]);
    return;
  }
...
`);
  ...
  // Write wrapper shell script
  const wrapperPath = path.join(home, 'cli-wrapper.sh');
...
case "$1" in
  --version)
    v=$("${cliPath}" --version 2>/dev/null &
    p=$!; sleep 2; kill $p 2>/dev/null; wait $p 2>/dev/null)
    echo "$v" | grep -E '^[0-9]+\\.' | head -1
    exit 0
    ;;
  --help|-h)
    "${cliPath}" --help 2>/dev/null &
    p=$!; sleep 3; kill $p 2>/dev/null; wait $p 2>/dev/null
    exit 0
    ;;
  *)
    exec node --import "${preludePath}" "${cliPath}" "$@"
    ;;
esac
`);
