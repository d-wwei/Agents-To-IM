...
> 状态：**待开始**
> 目标：将 CodePilot 飞书 V2 的核心能力移植到 agent-to-im-core 开源库

## 背景

CodePilot 已实现飞书 V2：CardKit v2 流式卡片、工具进度、权限内联按钮、Thinking 状态、Footer 耗时。
agent-to-im-core 当前飞书实现：无流式（发最终结果）、权限靠文本命令 `/perm`、无工具进度显示。
...
理由：
- agent-to-im-core 是轻量库，不像 CodePilot 有独立的 channel plugin 架构
