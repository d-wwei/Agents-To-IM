...
export class DiscordAdapter extends BaseChannelAdapter {
  ...
  private async registerSlashCommands(): Promise<void> {
    if (!this.client?.application || !discordJs) return;

    const { ApplicationCommandOptionType } = discordJs;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const commands: any[] = DISCORD_SLASH_COMMANDS.map((command) => ({
      name: command.name,
    ...
    try {
      ...
      console.warn('[discord-adapter] Failed to register global slash commands:', err instanceof Error ? err.message : err);
    }
    ...
    const guilds = this.client.guilds?.cache
      ? Array.from(this.client.guilds.cache.values()).filter((guild: { id: string }) =>
        allowedGuilds.length === 0 || allowedGuilds.includes(guild.id),
      )
      : [];

    await Promise.all(guilds.map(async (guild: { commands?: { set?: (value: unknown) => Promise<unknown> } }) => {
      if (!guild.commands?.set) return;
      try {
        await guild.commands.set(commands);
      } catch (err) {
        console.warn('[discord-adapter] Failed to register guild slash commands:', err instanceof Error ? err.message : err);
      }
    }));
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private isGuildAllowed(guild: any): boolean {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private buildSlashCommandText(interaction: any): string | null {
    const rawOptions = Array.isArray(interaction.options?.data) ? interaction.options.data : [];
    const normalized = Object.fromEntries(rawOptions.map((option: { name: string; value: string | boolean }) => [
      option.name,
      option.value,
    ]));
    return buildDiscordSlashCommandText(interaction.commandName, normalized);
  }
  ...
  private async processMessage(message: any): Promise<void> {
    ...
    if (isGuild) {
      if (!this.isGuildAllowed(message.guild)) {
        return;
      }
  ...
  private async handleInteraction(interaction: any): Promise<void> {
    if (interaction.isChatInputCommand?.()) {
      ...
      if (!this.isAuthorized(userId, chatId) || !this.isGuildAllowed(interaction.guild)) {
        try {
          if (interaction.isRepliable?.()) {
            await interaction.reply({ content: 'This command is not authorized here.', ephemeral: true });
          }
        } catch {
          // Ignore authorization reply failures
        }
        return;
      }
      ...
      try {
        await interaction.reply({ content: 'Command received. Result will be sent to this chat.', ephemeral: true });
      } catch {
        // Ignore reply failures, the command can still be processed
      }

      const inbound: InboundMessage = {
        messageId: `discord-command-${interaction.id}`,
        address: {
          channelType: 'discord',
          chatId,
          userId,
