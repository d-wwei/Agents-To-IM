...
Agent-to-IM-core is a host-agnostic bridge library that connects [Claude Code SDK](https://docs.anthropic.com/en/docs/claude-code/sdk) to IM platforms, allowing users to interact with Claude through Telegram, Discord, and Feishu (Lark).
...
Agent-to-IM-core was extracted from CodePilot as a standalone library for developers who want to embed the IM bridge capability in their own applications.
...
```bash
npm install agent-to-im-core
```

Or clone this repo and install dependencies:

```bash
git clone https://github.com/op7418/Agent-to-IM-core.git
cd Agent-to-IM-core
npm install
```
...
import type { BridgeStore, LLMProvider, PermissionGateway, LifecycleHooks } from 'agent-to-im-core/host';
